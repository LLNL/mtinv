...
set TITLE_TXT="2021-11-18 New Madrid, MO"
...
#### Best fitting DevMT
 echo " -2.54292 0.00000 " | gmt psxy $R $J -Ss0.18i -W0.5p,black -Gwhite -O -K -Vq >> ${PS}

#### This lune lat,lon for User supplied eigenvalues 
####
if( ${plt_user_pt} == "y" ) then
 echo " -0.71238 7.74413 " | gmt psxy $R $J -Sd0.15i -W1p,black -Gwhite -O -K -Vq >> ${PS}
...
-0.71238 7.74413 0 -0.079 0.781 -0.405 0.221 -0.291 0.539 24 -0.71238 7.74413 VR=85.7%
EOF

else

gmt psmeca $R $J -Sm0.25i/10p/3p -L1p,black -G100 -E255 -N -O -K -Vq >> ${PS} << EOF
-0.71238 7.74413 0 -0.079 0.781 -0.405 0.221 -0.291 0.539 24 -0.71238 7.74413    
EOF
...
####
echo " -0.71238 7.74413 " | gmt psxy $R $J -Sa0.3i -W1p,black -Gwhite -O -K -Vq >> ${PS}

#### VR label
####
if( ${plt_label_vr} == "y" ) then

 echo " -0.71238 7.74413 VR=85.7% " | gmt pstext $R $J -F+f10p,Helvetica-Bold,black+jML -D+0.15i/+0.15i -C0.01i -Gwhite -W0.1p,black -O -K -Vq >> ${PS}
...
0.9 0.95 1.0 -0.079 +0.781 -0.405 +0.221 -0.291 +0.539 24 0.9 0.95 (Full) VR=85.7
0.9 0.88 1.0 -0.148 +0.745 -0.597 +0.260 -0.232 +0.604 24 0.9 0.88 (Dev) VR=85.5
0.9 0.81 1.0 -0.098 +0.759 -0.662 +0.212 -0.255 +0.615 24 0.9 0.81 (DC) VR=84.5
EOF

gmt psmeca -R0/1/0/1 -JX4i/8.5i -Sm0.3i/12p/3p -L1p,black -G100 -E255 -N -O -K -Vq >> ${PS} << EOF
0.9 0.74 1.0 +1.000 +1.000 +1.000 +0.000 -0.000 -0.000 24  0.9 0.74 (+Iso) VR=-15.3
0.9 0.67 1.0 -1.000 -1.000 -1.000 +0.000 +0.000 +0.000 24  0.9 0.67 (-Iso) VR=-4.7
EOF
