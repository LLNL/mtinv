...
set TITLE_TXT="2009-12-22 Kambrata, Kyrgyzstan"
...
#### Best fitting DevMT
 echo " 5.20872 0.00000 " | gmt psxy $R $J -Ss0.18i -W0.5p,black -Gwhite -O -K -Vq >> ${PS}

#### This lune lat,lon for User supplied eigenvalues 
####
if( ${plt_user_pt} == "y" ) then
 echo " -19.95298 59.99158 " | gmt psxy $R $J -Sd0.15i -W1p,black -Gwhite -O -K -Vq >> ${PS}
...
-19.00083 60.43808 0 0.996 0.276 0.406 -0.053 0.016 -0.034 24 -19.00083 60.43808 VR=73.8%
EOF

else

gmt psmeca $R $J -Sm0.25i/10p/3p -L1p,black -G100 -E255 -N -O -K -Vq >> ${PS} << EOF
-19.00083 60.43808 0 0.996 0.276 0.406 -0.053 0.016 -0.034 24 -19.00083 60.43808    
EOF
...
####
echo " -19.00083 60.43808 " | gmt psxy $R $J -Sa0.3i -W1p,black -Gwhite -O -K -Vq >> ${PS}

#### VR label
####
if( ${plt_label_vr} == "y" ) then

 echo " -19.00083 60.43808 VR=73.8% " | gmt pstext $R $J -F+f10p,Helvetica-Bold,black+jML -D+0.15i/+0.15i -C0.01i -Gwhite -W0.1p,black -O -K -Vq >> ${PS}
...
0.9 0.95 1.0 +0.996 +0.276 +0.406 -0.053 +0.016 -0.034 24 0.9 0.95 (Full) VR=73.8
0.9 0.88 1.0 -0.092 -0.018 +0.110 -0.723 -0.613 -0.045 24 0.9 0.88 (Dev) VR=13.2
0.9 0.81 1.0 -0.157 -0.026 +0.183 -0.450 -0.876 +0.021 24 0.9 0.81 (DC) VR=-19.9
EOF

gmt psmeca -R0/1/0/1 -JX4i/8.5i -Sm0.3i/12p/3p -L1p,black -G100 -E255 -N -O -K -Vq >> ${PS} << EOF
0.9 0.74 1.0 +1.000 +1.000 +1.000 -0.000 +0.000 +0.000 24  0.9 0.74 (+Iso) VR=-1292.6
0.9 0.67 1.0 -1.000 -1.000 -1.000 -0.000 +0.000 -0.000 24  0.9 0.67 (-Iso) VR=-2640.1
EOF
